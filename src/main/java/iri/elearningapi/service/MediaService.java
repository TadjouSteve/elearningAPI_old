...
public class MediaService {
	...
	public Article createArticle(Article article,int idRubrique) {
		...
		return article;
			...
			throw new ElearningException(
					new ErrorAPI("La rubrique de l'article n'est pas definie....!"));
		}
	}
	...
	public Rubrique getRubrique(int idRubrique) {
		...
		if (rubriqueRepository.existsById(idRubrique)) {
			...
			throw new ElearningException(
					new ErrorAPI("Aucune rubrique journalistique trouver....!"));
		}
	}
	...
	private void controlRubrique(Rubrique rubrique) {
		if (rubrique.getNom()==null || rubrique.getNom().length()<5) {
			throw new ElearningException(
					new ErrorAPI("Le nom de la  rubrique ne doit pas être vide, ou contenir moins de trois(03) caractères", 0));
		}
	...
	private void controlArticle(Article article) {
		if (article.getTitre()==null || article.getTitre().length()<10) {
			throw new ElearningException(
					new ErrorAPI("Le Titre de l'article ne doit pas être vide, ou contenir moins de dix(10) caractères", 0));
		}
		...
		if (article.getTexte()==null || article.getTexte().length()<30) {
			throw new ElearningException(
					new ErrorAPI("Le Texte de l'article ne doit pas être vide, ou contenir moins de trente(30) caractères", 0));
		}
		
		if (article.getAuteur()==null || article.getAuteur().length()<10) {
			throw new ElearningException(
					new ErrorAPI("Le Nom de l'auteur ne doit pas être vide, ou contenir moins de dix(10) caractères", 0));
		}
	...
	public Article getArticleByRubriqueAndTitre(String titreArticle,String  nomRubrique) {
		if (rubriqueRepository.existsByNom(nomRubrique)) {
			if (articleRepository.existsByTitre(titreArticle)) {
				...
				throw new ElearningException(
						new ErrorAPI("L'article recherché n'existe pas....!"));
			}
		}else {
			throw new ElearningException(
					new ErrorAPI("La rubrique visée n'existe pas....!"));
		}
	}
	...
	public Page<Article> getAllArticleRubrique(int idRubrique,int pageNumber){
		...
		if (rubriqueRepository.existsById(idRubrique)) {
			pageArticle=articleRepository.findByRubriqueOrderByIdAsc(rubriqueRepository.findById(idRubrique).get(), pageable);
			return pageArticle; 
		}else {
			throw new ElearningException(
					new ErrorAPI("La rubrique visée n'existe pas....!"));
